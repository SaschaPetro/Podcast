...
**Takt 2** setzt auf den vorbereiteten offenen Themen auf: Der Moderator wählt aus allen offenen Themen die 5-6 wichtigsten aus und schreibt ein nachrichtenorientiertes Manuskript; die dabei verwendeten Original-Quellen werden dauerhaft in `episoden_quellen` festgehalten. Der Zielbereich liegt bei 1300-1450 Wörtern. Unter 1200 Wörtern wird automatisch ein zweiter vollständiger Entwurf angefordert; bleibt auch dieser zu kurz, wird der Lauf abgebrochen und keine unvollständige Episode gespeichert. Bevor daraus Audio erzeugt wird, prüft ein Faktencheck-Schritt das Manuskript gegen die Original-Quellen; nur bei Freigabe wird die MP3 erzeugt (als allererste Zeile im Manuskript steht dabei immer ein fester KI-Kennzeichnungshinweis, siehe Abschnitt 6). Die fertige MP3 wird lokal gespeichert UND zusätzlich in einen öffentlichen Supabase-Storage-Bucket hochgeladen, damit eine stabile URL unabhängig von GitHub-Actions-Artefakten oder dem lokalen Rechner existiert. Unabhängig davon prüft alle 4 Episoden ein Rhetorik-Agent die zuletzt erschienenen Manuskripte auf Wiederholungen, Nachrichtenvorrang, unnötiges Storytelling und Zielwortzahl - rein informativ, ohne automatische Änderung.
...
Ein Agent: **Rhetorik-Check** - unabhängig vom Faktencheck (der nur Fakten prüft, siehe Abschnitt 5, Punkt 7), prüft dieser Agent die *rhetorische* Qualität über mehrere Episoden hinweg: wiederholte Formulierungen und Satzmuster, variierende Einstiege/Übergänge/Abschlüsse, ungefähr 1300-1450 Wörter sowie den klaren Vorrang des Nachrichtenkerns. Er beanstandet insbesondere erfundene Figuren oder Dialoge, ausgedehnte Alltagsszenen, künstliche Spannungskurven, atmosphärische Einstiege vor der Meldung und jedes Storytelling, das für die Erklärung nicht notwendig ist.
...
**5. Aktuelle Version:** Version 7 ist aktiv und wurde bewusst als menschliche Version freigeschaltet. Sie schreibt Nachrichtenvorrang und 1300-1450 Wörter verbindlich vor. Storytelling ist erst nach Meldung und Kontext erlaubt, ausschließlich bei echtem Erklärungsbedarf. Die automatisch vom Rhetorik-Agenten vorgeschlagenen Versionen 2-5 sind inaktiv; Version 6 bleibt als vorherige menschliche Fassung für einen möglichen Rücksprung erhalten.
...
| `LÄNGE` | Ziel-Wortzahl 1300-1450 Wörter für ungefähr 10 Minuten; unter 1200 Wörtern wird automatisch einmal neu generiert und danach notfalls abgebrochen |
...
**Technische Längensicherung:** `erstelle_manuskript()` zählt die tatsächlich ausgegebenen Wörter ohne die abschließende Themen-ID-Zeile. Liegt der erste Entwurf unter 1200 Wörtern, wird ein zweiter vollständiger Entwurf angefordert. Dieser darf nur durch journalistische Substanz erweitert werden: belegte Details, Hintergrund, Zusammenhänge, Folgen für Unternehmen und Handlungsmöglichkeiten. Bleibt auch Versuch 2 unter 1200 Wörtern, löst die Funktion einen Fehler aus; die Episode wird nicht gespeichert und nicht vertont.
...
`GEMINI_MODEL_NAME` steuert zentral, welches Gemini-Modell `generiere_episode.py`, `recherche_und_redaktion.py`, `verarbeite_rohnachricht.py` und `rhetorik_check.py` für Text/Redaktion verwenden. Der aktuell lokal konfigurierte Modellname ist `gemini-3.5-flash-lite`; in GitHub Actions zählt der dort als Secret hinterlegte Wert.
