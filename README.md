...
| TTS-Erzeugung schlägt fehl (API-Fehler, z.B. Netzwerk/Kontingent) | `_via_gemini_tts()`/`_via_deepgram()`/`_via_elevenlabs()` haben **kein eigenes** Try/Except - die Exception propagiert bis `fuehre_schritt_aus()` hoch, Schritt 3/4 wird "fehlgeschlagen". Da `erzeuge_audio_fuer_episode()` den DB-Update-Aufruf dadurch nie erreicht, bleiben `episoden.audio_pfad` **und** `audio_url` unverändert (meist `NULL`) - `_schreibe_atomar()` schreibt ohnehin immer erst nach vollständiger Erzeugung, es entsteht also nie eine unvollständige Datei lokal. | `generiere_audio.py`, `_via_gemini_tts()`/`_via_deepgram()`/`_via_elevenlabs()` |
| **Gemini TTS: MP3-Konvertierung schlägt fehl** (z.B. `lameenc` liefert bei defekten/leeren PCM-Daten einen Fehler) | Ebenfalls **kein eigenes** Try/Except in `_via_gemini_tts()` - dieselbe Kette wie oben: Schritt 3/4 "fehlgeschlagen", `audio_pfad`/`audio_url` bleiben `NULL`. **Kein WAV-Fallback und kein automatischer Rückfall auf Deepgram** - die Konvertierung passiert komplett im Speicher (`_pcm_zu_mp3()`), vor `_schreibe_atomar()`; schlägt sie fehl, wird gar nichts geschrieben, auch keine Notlösung in einem anderen Format. Manuelle Behebung: `AUDIO_ANBIETER = "deepgram"` in `morgenlauf.py` setzen (oder `anbieter="deepgram"` bei einem manuellen `text_zu_audio()`-Aufruf, siehe Abschnitt 7) und den Audio-Schritt für die betroffene Episode erneut auslösen. | `generiere_audio.py`, `_via_gemini_tts()`/`_pcm_zu_mp3()` |
